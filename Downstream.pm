...
sub run {
    ...
    my $unshifted_start = $start > $end ? $tv->cds_start_unshifted - 1 : $tv->cds_start_unshifted;
    my $substr_start = $unshifted_start - ( ( ( $unshifted_start - 1 ) % 3 ) + 1 );

    my $downstream_seq = substr($cds_seq, $substr_start > 0 ? $substr_start : 0);
